...
- Type/paste/IME at the composer caret; **Enter** saves and queues, **Shift+Enter** inserts a newline. The composer expands from 1 to 6 text rows (3–8 including borders), scrolling to keep the caret visible. **Left/Right** move by complete Unicode graphemes; **Ctrl+Left/Right** move by whitespace-delimited words. **Home/End** move to logical line boundaries; **Ctrl+Home/End** to the entire draft's boundaries. **Backspace/Delete** remove the grapheme before/at the caret; **Ctrl+W** (also Ctrl+Backspace) removes the previous word. Clicking composer text places the caret; both cells of a wide glyph snap to its start. No slash commands, completion or hosted Pi terminal are implied.
- Unmodified **Up/Down** scroll the transcript by one display row, even with a draft; they do not move the composer caret. **PgUp/PgDn** retain five-row scrolling, wheel retains three-row scrolling. Scroll is clamped to the computed transcript viewport. Panel-focus modifier shortcuts and room toggle retain precedence. Keys held before entry still release to their original terminal; room presses/repeats/text never enter its hidden PTY.
- Left-drag transcript text to select; release finalizes, a plain click clears the previous selection. **Ctrl+C / Ctrl+Shift+C** copy only that selection through the existing asynchronous clipboard-event path (never a terminal interrupt). Wide/combining/emoji graphemes remain intact; dragging outside the transcript clamps to its visible edges and cannot activate tabs/panes. Selection remains anchored to cached transcript rows during scrolling/appends; width rewrap clears it predictably. Copy includes selected attribution/text, **preserves soft-wrap newlines**, and excludes app chrome and control bytes. Leaving the room hides selection/caret; re-entering clears selection.
...
- While the room is open, **all current workspace room-member agents are highlighted in the existing sidebar**, with compact name + stable public pane ID rows. Plain shells and other workspaces are not highlighted. Current cached runtime identity is checked with constant-time lookups, not render-time member scans/API calls. Closing the room restores the original custom topic/model/name rows and focused-pane styling. Sidebar clicks still focus real terminal panes. Membership API reads remain immediately current; delivery presentation refreshes outside render at most four times per second.
...
The extension registers `/room-enable`, `/room-disable` and `room_reply`. Pi TUIs automatically connect when launched/reloaded with an exact absolute `HERDR_SOCKET_PATH` and `HERDR_PANE_ID`. Outside Herdr, or with `HERDR_ROOM_ENABLED=0`, the receiver starts inactive and opens no sockets/timers. `room_reply` requires a matching active delivery.
