...
- Agent messages display only **`name (pane.id)`** as their header, e.g. `Aporia (wB:p2)`. Human messages have no header and use a highlighted background. Sequence IDs, recipients, expiry and reply correlation remain in the API/storage, not conversation headers.
