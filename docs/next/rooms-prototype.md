...
- The room is a normal group conversation: transcript and bottom Pi-style composer, with only error/delivery status when present. There is no metadata header, member listing/count or recipient picker. **Enter broadcasts** to current workspace agents, ignoring and clearing legacy UI targets — unless the text *starts* with `@Name` mentions (2026-09-17): then only those members are addressed (`@Keystone @Sift question…`; multi-word names plainly `@Tuning Fork`, quoted `@"Tuning Fork"`, or joined `@Tuning.Fork`; a pane id `@w4:pE` also works; names match case-insensitively with icons/markup stripped). Unknown, ambiguous or session-less mentions reject the post with a status message and keep the draft; a mention never silently becomes a broadcast. The mentions remain in the transcript text. Later joiners never receive old questions. **Tab/BackTab** do not select recipients or focus a hidden pane; configured global panel shortcuts are unchanged. Agents without live session identity are reported unavailable rather than silently omitted.
...
- `room.post {workspace_id, text, recipient?, recipients?}` → a stored human-owner question. Omitted recipient snapshots all current members; explicit recipient is `{pane_id, terminal_id, session}` taken from `room.get`, validated again at acceptance. `recipients` (additive, 2026-09-17) is a list of such entries for an addressed group question; it takes precedence over `recipient`, and every entry must be a current member with live session identity or the whole post fails with `invalid_recipient` and nothing is saved or delivered. The transcript's `recipients` captures session-identified broadcast targets; runtime unavailable entries also show visible members lacking a live session. An empty room stores a question with no addressed audience and never replays it. A request is identified by **room ID + its message sequence** and expires after ten minutes.
...
After successful registration, the fresh TypeScript entrypoint attempts one **automatic named introduction**, `Hi, I'm <name>.`, on EVERY startup/reload/resume/re-enable without starting a model turn. It uses current exact-session membership metadata; without a published name it posts nothing. The agent chooses and publishes its name first, then posts its named introduction. This uses an ordinary agent post, NOT the optional server-side deduplicated arrival mode. The adapter attempts at most once per receiver lifecycle (not every heartbeat/turn); deliberate reload starts a fresh introduction even for the same session. Arrival API errors/uncertain acknowledgements produce a warning but never poison successful registration or retry on heartbeat. Old servers may lack the new method: questions still work, but arrivals/posts require the candidate server. `--no-session` remains explicitly volatile and is not reported as persisted tool success.
