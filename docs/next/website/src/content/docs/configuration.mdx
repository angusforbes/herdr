...
`focus_panel_next` and `focus_panel_previous` cycle focus forward or backward through Spaces → Agents → terminal → Search, including Search only when it is open. The four search navigation shortcuts work only while search is open, in normal modes (not dialogs or other modal input): `search_result_previous` / `search_result_next` select and visit results while retaining keyboard focus, `search_input` focuses the query (so Enter submits it), and `search_mode` switches keyword / AI search. They do not open Search; use `search_pane` for that.

These direct shortcuts require a terminal that reports the corresponding modified keys distinctly; remap them if your terminal intercepts or conflates them.
