...
### Shared context + clean question rendering (pending)

User asked via room request58 for real shared conversation context, then directly requested agent pane display only human question, not JSON. Parent implemented checkout-only Pi `room-context.mjs` adapter plus index.ts integration: each successful claim fetches same immutable sequence-bounded pre-question history (last40, JSON32KiB suffix), attributed agent names/panes/reply links; no private conversations; no replay/fanout. Added read-only current-bound room_read tool for earlier/newer messages. Fresh TS wraps old cached native receiver rather than depending on native module reload. New custom renderer uses question-only details, parses legacy question envelope as fallback; no protocol/history displayed, though model/session context retains them. All25 Node tests and11 Python tests pass, including actual Pi hot-loader tests. Not committed/deployed, no real model context test yet; no agents spawned or messages dispatched. Passive post-question reply streaming is not implemented; next question/explicit room_read sees new replies.

### Human block padding verified by parent (pending)

Completed partial roompad-build: human grey blocks now have one blank top/bottom row and one visual column inset at widths>=3; agent headers/body unchanged. Role-aware hit mapping leaves copied text unindented. Parent fixed invalid reply test fixture, made human full-row style survive blank/clipped glyph layout, and added cross-role/reversed/scrolled Unicode selection test. TestBackend wide-glyph continuation-cell resets explicitly excluded while checking leading glyph/padding styles. All78 room tests pass; debug build, cargo fmt, diff check pass. Not committed/deployed.

### Bounded agent posts + automatic arrival notice (checkout-only candidate)

Added neutral `room.agent.post` and Pi `room_post {text}`: exact live workspace/pane/terminal/session validation, server-resolved immutable author/name, 8192-byte/nonblank bounds, saved-before-ack rollback, no recipient/reply/expiry, no delivery fanout. The tool captures binding through the receiver's serialized queue, requires no active question, does not terminate unrelated work, and never silently retries ambiguous writes. Human broadcasts and single-question `room_reply` remain unchanged.

Fresh `index.ts` wraps successful receiver registration to attempt deterministic `Joined the room.` once per lifecycle, without a Pi message/model turn. Both native adapters may remain cached across reload. Persisted `Message.arrival` defaults false for legacy snapshots; dedup uses workspace + actual agent label/session, ignoring terminal/pane/name replacement. Duplicates validate current membership first, return the original sequence and skip save/append even when full. New sessions/workspaces can announce. Old-server/failed/uncertain arrival acknowledgements warn but do not freeze registration; deliberate reconnect is safe via server dedup. Ordinary posts have no idempotency key. Existing history snapshots/read naturally include these agent records; no ongoing-turn injection or conversation loop.

Verification: **85 Rust room tests, 34 Node tests, 11 Python tests**, generated-schema check and debug build passed; `cargo fmt --check`/`git diff --check` passed. Python includes real installed Pi loader/reload with owned fake sockets and no model requests. Tests cover cached adapters, no-model arrival, binding races, no-delivery posts, no retries/false success, full-room/terminal-replacement dedup, legacy defaults and persistence failure. Updated real-model smoke transcript expectations for two arrival notices, but only syntax-checked that opt-in script; no real-model/production/physical UI tests, full suite or release build run. Logs: `/tmp/room-agent-post-{rust-final,node,python,schema,build}.log`.

Preserved all parent's pre-existing human-padding files/docs. No global/live edits, deployment/handoff, commit/push or child agents. Parent owns review and deployment; new API requires the candidate server, while old-server arrival failure leaves receiver questions usable.

### Every-reload introduction override installed

Angus explicitly requested a fresh introduction EVERY reload. Fresh TS captures published name from exact current room.get membership, posts ordinary agent contribution once per lifecycle (no arrival=true dedup, no model turn). Generic introduction only if name unpublished; global instructions also updated. 34 Node +11 Python tests pass including real Pi reload emitting two introductions. Installed only index.ts with backup /home/agf/.local/state/herdr-upgrades/room-every-reload-20260917-194720; no server handoff, changes not yet committed.
