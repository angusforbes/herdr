...
use crate::api::schema::{
    AgentConversationParams, AgentForkParams, AgentSessionInfo, ErrorBody, PaneInfo,
    PaneSplitParams, ResponseResult, SplitDirection, SuccessResponse,
};
use crate::app::App;
    ...
    pub(super) fn handle_agent_fork(&mut self, id: String, params: AgentForkParams) -> String {
        ...
        let pane = match serde_json::from_str::<SuccessResponse>(&split) {
            ...
            _ => {
                return encode_error(
                    id,
                    "conversation_fork_split_failed",
                    format!(
        ...
        let (agent, argv) = match self.queue_conversation_launch(&pane.pane_id, name, args) {
            ...
            Err(message) => {
                return encode_error(
                    id,
                    "conversation_fork_launch_failed",
                    format!(
