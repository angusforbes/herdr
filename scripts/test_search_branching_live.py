...
def main():
    ...
    parser.add_argument('--pi', help='absolute Pi executable; bypass tool-manager shims in the isolated fixture')
    ...
    with tempfile.TemporaryDirectory(prefix='herdr-branch-smoke-') as root:
        ...
        if args.pi:
            pi_binary = Path(args.pi).resolve(strict=True)
            fixture_bin = root / 'bin'
            fixture_bin.mkdir()
            (fixture_bin / 'pi').symlink_to(pi_binary)
        ...
        try:
            ...
            reporter = Path(__file__).resolve().parents[1] / 'src/integration/assets/pi/herdr-agent-state.ts'
            request('agent.start', {'name': 'fixture-pi', 'kind': 'pi', 'pane_id': source_pane, 'args': ['--offline', '--no-extensions', '--extension', str(reporter), '--no-skills', '--no-context-files', '--no-prompt-templates', '--no-tools', '--session', str(source)]})
            ...
            until(lambda: any(p.get('pane_id') == source_pane and (p.get('agent_session') or {}).get('value') == str(source) for p in request('pane.list', {})['panes']))
            ...
            # Disable the PTY line discipline before sending Ctrl+S/Ctrl+N;
            # otherwise XON/XOFF or canonical input can consume test keystrokes.
            tty.setraw(slave)
            fcntl.ioctl(slave, termios.TIOCSWINSZ, struct.pack('HHHH', 40, 140, 0, 0))
            def attach_fixture_tty():
                os.setsid()
                fcntl.ioctl(0, termios.TIOCSCTTY, 0)
            client = subprocess.Popen([binary, 'client'], env=dict(env, TERM='xterm-256color'), cwd=root, stdin=slave, stdout=slave, stderr=slave, preexec_fn=attach_fixture_tty)
            os.close(slave)
            captured = bytearray()
                ...
                while time.monotonic() < deadline and client.poll() is None:
                    ...
                    if ready:
                        try:
                            ...
                            break
            def wait_for(predicate, timeout=10, label='predicate'):
                """Read client output until *predicate(captured)* is true."""
                deadline = time.monotonic() + timeout
                while time.monotonic() < deadline and client.poll() is None:
                    ready, _, _ = select.select([master], [], [], .1)
                    if ready:
                    ...
                    if predicate(captured):
                        return
                raise RuntimeError(f'timeout waiting for {label}; captured tail: {bytes(captured[-4000:])!r}')
            try:
                # Wait until the client has rendered the workspace, proving
                # connection + initial frame blit are complete.
                wait_for(lambda c: b'branch-fixture' in c, timeout=10, label='TUI workspace render')
                read_client_for(.3)  # settle after initial render
                os.write(master, b'\x13')  # Ctrl+S: Herdr search, not source Pi input
                # Search readiness is not yet established by this harness:
                # generic help text such as " / " is not a valid predicate.
                read_client_for(.3)
                os.write(master, b'HERDR_BRANCH_DRAFT_UNSENT\r')
                read_client_for(1)
                os.write(master, b'\x0e\r')  # Ctrl+N selects first result, Enter activates preview
                read_client_for(2)
                assert b'Pi tree' in captured, f'attached client did not render message preview; terminal bytes: {bytes(captured[-12000:])!r}'
            finally:
                client.terminate()
            ...
            log.seek(0)
            print(log.read()[-6000:])
            # This server owns only disposable fixture panes. Capture their
            # terminal output before cleanup so startup failures are diagnosable.
            try:
                for pane in request('pane.list', {})['panes']:
                    text = request('pane.read', {'pane_id': pane['pane_id'], 'source': 'recent_unwrapped', 'format': 'text'})['read']['text']
                    print(f"Fixture pane {pane['pane_id']}:\n{text[-6000:]}")
            except (OSError, RuntimeError, KeyError) as diagnostic_error:
                print(f'Could not capture fixture panes: {diagnostic_error}')
            raise
        finally:
            try:
                request('server.stop', {})
