    ...
    def test_room_saved_before_ack_and_restored_without_agent_dispatch(self):
        ...
        with tempfile.TemporaryDirectory(prefix="herdr-rooms-smoke-") as directory:
            with disposable.server(directory) as env:
                ...
                self.assertEqual(info["outbound_delivery"], "pi_polling_at_most_once")
                self.assertEqual(info["receivers"], [])
                self.assertEqual(info["deliveries"], [])
            ...
            with disposable.server(directory) as env:
                ...
                self.assertEqual(restored["receivers"], [])
                self.assertEqual(restored["deliveries"], [])
